...
import { loadSessionTranscriptForRun } from "./dashboard-session-transcript.js";
...
function buildTaskflowRunEntry(flowRow, options = {}) {
  ...
  const sessionTranscript = loadSessionTranscriptForRun(state, options);
  if (normalizeString(sessionTranscript.firstVisibleUserText) && isInternalTaskPrompt(state.initialUserPrompt)) {
    state.initialUserPrompt = normalizeString(sessionTranscript.firstVisibleUserText);
    state.promptText = state.initialUserPrompt;
    state.normalizedPromptText = state.promptText;
  }
  if (Array.isArray(sessionTranscript.timelineEvents) && sessionTranscript.timelineEvents.length > 0) {
    state.timelineEvents = [...state.timelineEvents, ...sessionTranscript.timelineEvents].slice(-40);
  }
  if (!normalizeString(state.lastExternalMessage) && normalizeString(sessionTranscript.lastExternalMessage)) {
    state.lastExternalMessage = normalizeString(sessionTranscript.lastExternalMessage);
  }
