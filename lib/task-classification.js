...
function isSystemTriggeredPrompt(prompt) {
  const raw = normalizeString(prompt).toLowerCase();
  if (!raw) return false;
  return (
    raw.startsWith("system (untrusted):") ||
    raw.includes("an async command you ran earlier has completed") ||
    raw.includes("handle the result internally. do not relay it to the user") ||
    raw.includes("<<<begin_openclaw_internal_context>>>") ||
    raw.includes("[internal task completion event]") ||
    raw.includes("[subagent context]") ||
    raw.includes("openclaw runtime context (internal)")
  );
}
...
function classifyOrchestrationMode(cfg, currentAgentId, prompt, pluginConfig = null) {
  ...
  if (!task) return "solo";
  if (isSystemTriggeredPrompt(prompt)) return "solo";
...
function classifyMissionEntryMode(cfg, currentAgentId, prompt, pluginConfig = null) {
  ...
  if (isSystemTriggeredPrompt(prompt)) return "plain";
