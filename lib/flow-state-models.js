...
function mergeFlowTasksIntoDurable(durable, flowTasks = []) {
  ...
  for (const task of flowTasks) {
    ...
    const merged = {
      ...(matchIndex >= 0 ? childTasks[matchIndex] : {}),
      ...(taskId ? { taskId } : {}),
      ...
      ...(phase ? { phase } : {}),
      ...(normalizeString(task?.terminalSummary) ? { progressSummary: normalizeString(task.terminalSummary) } : {}),
      ...(normalizeString(task?.progressSummary) ? { progressSummary: normalizeString(task.progressSummary) } : {})
    };
