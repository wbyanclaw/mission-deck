import { FLOW_STATES } from "./contracts.js";
import { hasNonEmptyString, isoNow, looksLikeAwaitingUserInputReply, normalizeString } from "./text-helpers.js";

function cloneSimple(value) {
  return JSON.parse(JSON.stringify(value));
}

function isCompletedTaskPhase(phase) {
  return ["reported", "succeeded", "success", "completed", "done", "delivered", "blocked", "failed", "cancelled", "timed_out", "timeout"].includes(phase);
}

function countChildEvidence(childTasks = []) {
  const keys = new Set();
  for (const task of childTasks) {
    const phase = normalizeString(task?.phase).toLowerCase();
    if (!isCompletedTaskPhase(phase)) continue;
...
function deriveChildPhaseFromTaskView(task = {}) {
  const deliveryStatus = normalizeString(task?.deliveryStatus).toLowerCase();
...
function mergeFlowTasksIntoDurable(durable, flowTasks = []) {
  if (!durable || !Array.isArray(flowTasks) || flowTasks.length === 0) return durable;
  const childTasks = Array.isArray(durable.childTasks) ? cloneSimple(durable.childTasks) : [];
  for (const task of flowTasks) {
    const taskId = normalizeString(task?.id);
    const childSessionKey = normalizeString(task?.childSessionKey);
    const childRunId = normalizeString(task?.runId);
    if (!taskId && !childSessionKey && !childRunId) continue;
    const phase = deriveChildPhaseFromTaskView(task);
    const matchIndex = childTasks.findIndex((candidate) =>
      (taskId && normalizeString(candidate?.taskId) === taskId) ||
      (childSessionKey && normalizeString(candidate?.childSessionKey) === childSessionKey) ||
      (childRunId && normalizeString(candidate?.childRunId) === childRunId)
    );
    const existing = matchIndex >= 0 ? childTasks[matchIndex] : {};
    const existingPhase = normalizeString(existing?.phase).toLowerCase();
    const nextPhase = normalizeString(phase).toLowerCase();
    const preserveTerminalResult = isCompletedTaskPhase(existingPhase) && nextPhase && !isCompletedTaskPhase(nextPhase);
    const merged = {
      ...existing,
      ...(taskId ? { taskId } : {}),
      ...(normalizeString(task?.agentId) ? { agentId: normalizeString(task.agentId) } : {}),
      ...(childSessionKey ? { childSessionKey } : {}),
      ...(childRunId ? { childRunId } : {}),
      ...(normalizeString(task?.label) ? { label: normalizeString(task.label) } : {}),
      ...(phase && !preserveTerminalResult ? { phase } : {}),
      ...(normalizeString(task?.terminalSummary) && !preserveTerminalResult ? { progressSummary: normalizeString(task.terminalSummary) } : {}),
      ...(normalizeString(task?.progressSummary) && !preserveTerminalResult ? { progressSummary: normalizeString(task.progressSummary) } : {})
    };
    if (matchIndex >= 0) childTasks[matchIndex] = merged;
    else childTasks.push(merged);
  }
  ...
  durable.receivedEvidenceCount = countChildEvidence(childTasks);
  return durable;
}
...
export function buildCanonicalFlowState({ runId, sessionKey, parentLink = null, entryMode = "plain", orchestrationMode = "solo", orchestrationPlan = null, initialUserPrompt = "" }) {
  return {
    schemaVersion: 1,
    ...
    lastFailureKind: "",
    lastFailureReason: "",
    initialUserPrompt: normalizeString(initialUserPrompt),
...
export function applyDurableFlowToRun(state, flow = null) {
  ...
  mergeFlowTasksIntoDurable(durable, flow?.tasks);
