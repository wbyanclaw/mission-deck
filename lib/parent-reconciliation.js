...
function findMatchingChildTask(tasks = [], childOutcome = {}) {
  const childTaskId = normalizeString(childOutcome.childTaskId);
  const childRunId = normalizeString(childOutcome.childRunId);
  const childSessionKey = normalizeString(childOutcome.childSessionKey);
...
function applyChildOutcomeToParent({
  ...
  const currentCanonicalParentState = buildCanonicalParentState(parentTaskFlow, parentState, null, updateDurableChildTask);
  const existingTask = findMatchingChildTask(currentCanonicalParentState.childTasks, childOutcome);
  const duplicateTerminalOutcome = Boolean(
    existingTask &&
    !hasNonEmptyString(childOutcome.finalReplyText) &&
    isTerminalChildPhase(existingTask.phase) &&
    normalizeString(existingTask.phase).toLowerCase() === normalizeString(childOutcome.phase).toLowerCase()
  );
  if (duplicateTerminalOutcome) {
    applyCanonicalParentState(parentState, currentCanonicalParentState);
    return {
      nextState: normalizeString(parentState.flowCurrentStep || parentState.flowStatus).toLowerCase(),
      completed: normalizeString(parentState.flowCurrentStep).toLowerCase() === FLOW_STATES.COMPLETED,
      deliveryText: buildParentDeliveryText(parentState),
      duplicate: true
    };
  }
