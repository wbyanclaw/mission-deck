...
export function createDispatchHandler(deps) {
  ...
  return async function onAfterToolCall(event, ctx) {
    ...
    const conflictingLink = findConflictingChildLink(childLink);
    if (conflictingLink) {
      const conflictSummary = `child lane already belongs to flow ${normalizeString(conflictingLink.parentFlowId) || "(unknown)"}`;
      state.lastBlockReason = conflictSummary;
      appendTimelineEvent(state, {
        role: "异常摘要",
        owner: agentId,
      ...
      transitionFlow(taskFlow, state, "setWaiting", {
        ...
        blockedSummary: conflictSummary,
        waitJson: {
          kind: "child_link_conflict",
        ...
        stateJson: {
          ...
          lastFailureKind: "child_link_conflict",
          lastFailureReason: conflictSummary
        }
      }, canonicalEvent, "child link conflict");
      await dashboard.pushBlocker({
        timestamp: isoNow(),
        runId,
        agentId,
        reason: conflictSummary,
        toolName
      });
      await flushRun(runId, agentId, state);
      return;
    ...
    childLink.childTaskId = created.task.taskId;
