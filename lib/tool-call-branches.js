...
async function handleExecutionLaneTool({
  ...
  if (toolName === SESSIONS_SEND_TOOL_NAME) {
    ...
    const isDirectSoloChat = normalizeString(state?.entryMode) !== "mission-flow" || normalizeString(state?.orchestrationMode) === "solo";
    if (pluginConfig?.blockInvalidSessionsSend !== false && hasAgentId && !hasSessionKey && !hasLabel && !isDirectSoloChat) {
      return blockToolCall({
        dashboard,
        touchRun,
        transitionFlow,
