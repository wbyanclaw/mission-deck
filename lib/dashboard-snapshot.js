...
export function buildDashboardSnapshot(state, options = {}) {
  ...
  const activeCandidates = [...runtimeActiveRuns, ...taskflowActiveRuns]
    .map((entry) => options.normalizeHistoricalRunStatus(entry))
  ...
  const recentCandidates = [...runtimeRecentRuns, ...taskflowRecentRuns]
    .map((entry) => options.normalizeHistoricalRunStatus(entry))
    .sort((a, b) => String(b.updatedAt || b.startedAt || "").localeCompare(String(a.updatedAt || a.startedAt || "")))
    .slice(0, options.maxRecentRuns * 2);
  const visibleActiveRuns = mergeLiveRuns(activeCandidates, recentCandidates)
    .filter((entry) => isVisibleRootTaskflowRun(entry));
  const activeRuns = visibleActiveRuns.slice();
  const visibleRecentRuns = mergeVisibleRootRuns([...activeCandidates, ...recentCandidates])
    .filter((entry) => !activeRuns.some((active) => String(active.flowId || active.runId || "") === String(entry.flowId || entry.runId || "")))
    .slice(0, options.maxRecentRuns);
  const recentRuns = visibleRecentRuns;
  const rebuiltDirectSessions = Array.isArray(state.rebuiltDirectSessions) ? state.rebuiltDirectSessions : [];
  const directSessions = mergeDirectSessions([...runtimeActiveRuns, ...runtimeRecentRuns, ...rebuiltDirectSessions]);
  const liveRuns = activeRuns;
