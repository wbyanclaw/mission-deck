...
function getRunDedupeKey(entry) {
  const flowKey = normalizeString(entry?.flowId);
  if (flowKey) return flowKey;
  return normalizeString(entry?.runId);
}

function mergeTimelineEvents(entries) {
  const seen = new Set();
  const merged = [];
  for (const entry of entries) {
    for (const item of (Array.isArray(entry?.timelineEvents) ? entry.timelineEvents : [])) {
      const key = [
      ...
      if (!normalizeString(item?.text) || seen.has(key)) continue;
      seen.add(key);
      merged.push(item);
    }
  }
...
function mergeRunBuckets(entries) {
  const merged = new Map();
  for (const entry of entries) {
    const dedupeKey = getRunDedupeKey(entry);
  ...
  return Array.from(merged.values())
    .map((bucket) => chooseCanonicalFlowFragment(bucket))
    .filter(Boolean)
...
function filterVisibleRootRuns(runs) {
  return runs.filter((run) => isVisibleRootTaskflowRun(run));
}

function mergeVisibleRootRuns(entries) {
  return filterVisibleRootRuns(mergeRunBuckets(entries));
}
...
function looksLikeSystemPrompt(value) {
  ...
  return (
    ...
    text.includes("[cron:") ||
    text.includes("system (untrusted):") ||
...
function isVisibleDirectSession(run) {
  if (!run?.engineeringTask) return false;
  if (normalizeString(run?.flowId)) return false;
  if (normalizeString(run?.parentFlowId)) return false;
  if (normalizeString(run?.runId).startsWith("session:")) return true;
  const promptText = normalizeString(run?.initialUserPrompt || run?.promptText);
  if (!promptText) return false;
  return !looksLikeSystemPrompt(promptText);
}

function getDirectSessionKey(run) {
  return normalizeString(run?.sessionKey) || normalizeString(run?.runId);
}

function chooseDirectSession(entries) {
  const sorted = [...entries].sort((a, b) => String(b.updatedAt || "").localeCompare(String(a.updatedAt || "")));
  const rebuilt = sorted.find((run) => normalizeString(run?.runId).startsWith("session:"));
  const newest = sorted[0] || null;
  if (!newest) return null;
  const preferred = rebuilt || newest;
  const timelineEvents = mergeTimelineEvents(sorted);
  return {
    ...newest,
    ...preferred,
    sessionTitle: normalizeString(preferred?.sessionTitle) || normalizeString(newest?.sessionTitle),
    promptText: normalizeString(preferred?.promptText) || normalizeString(newest?.promptText),
    initialUserPrompt: normalizeString(preferred?.initialUserPrompt) || normalizeString(newest?.initialUserPrompt),
    originUserPrompt: normalizeString(preferred?.originUserPrompt) || normalizeString(newest?.originUserPrompt),
    lastExternalMessage: normalizeString(newest?.lastExternalMessage) || normalizeString(preferred?.lastExternalMessage),
    userAskedAt: normalizeString(preferred?.userAskedAt) || normalizeString(newest?.userAskedAt),
    updatedAt: normalizeString(newest?.updatedAt) || normalizeString(preferred?.updatedAt),
    startedAt: normalizeString(preferred?.startedAt) || normalizeString(newest?.startedAt),
    sessionKey: normalizeString(preferred?.sessionKey) || normalizeString(newest?.sessionKey),
    agentId: normalizeString(preferred?.agentId) || normalizeString(newest?.agentId),
    runId: normalizeString(preferred?.runId) || normalizeString(newest?.runId),
    timelineEvents
  };
}

function mergeDirectSessions(entries) {
  const buckets = new Map();
  for (const entry of entries) {
    const dedupeKey = getDirectSessionKey(entry);
    if (!dedupeKey) continue;
    const bucket = buckets.get(dedupeKey) || [];
    bucket.push(entry);
    buckets.set(dedupeKey, bucket);
  }
  return Array.from(buckets.values())
    .map((bucket) => chooseDirectSession(bucket))
    .filter((run) => isVisibleDirectSession(run))
...
export {
  ...
  isVisibleRootTaskflowRun,
  calculateAutonomyScore,
  calculateSuccessRate,
  mergeDirectSessions,
  mergeLiveRuns,
  mergeVisibleRootRuns,
  summarizeAgentLoad,
  summarizeChildTasks,
  summarizeFlowHealth
};
