...
import { loadDashboardTaskflowRuns } from "./dashboard-taskflow-rebuild.js";
import { loadDashboardDirectSessions } from "./dashboard-direct-session-rebuild.js";
import {
...
const DEFAULT_FLOW_REGISTRY_PATH = "/root/.openclaw/flows/registry.sqlite";

function isTerminalChildPhase(phase) {
  return ["completed", "succeeded", "success", "done", "reported", "delivered", "blocked", "failed", "timeout", "timed_out", "cancelled"]
    .includes(normalizeString(phase).toLowerCase());
}

function matchesChildOutcome(task, outcome) {
  return (
    ...
    normalizeString(task?.childRunId) === normalizeString(outcome?.childRunId)
  );
}

function isDuplicateTerminalChildOutcome(run, outcome) {
  const task = (Array.isArray(run?.childTasks) ? run.childTasks : []).find((entry) => matchesChildOutcome(entry, outcome));
  if (!task) return false;
  return (
    isTerminalChildPhase(task.phase) &&
    normalizeString(task.phase).toLowerCase() === normalizeString(outcome?.phase).toLowerCase()
  );
}
...
function createDashboardStore(logger, options = {}) {
  ...
  const flowRegistryPath = normalizeString(options.flowRegistryPath) || DEFAULT_FLOW_REGISTRY_PATH;
  const state = {
    activeRuns: new Map(),
    recentRuns: [],
    recentDispatches: [],
  ...
  async function flush() {
    ...
    await prune();
    try {
      const rebuilt = loadDashboardTaskflowRuns({
        ...options,
        flowRegistryPath,
        maxRecentRuns: MAX_RECENT_RUNS
      });
      state.taskflowActiveRuns = rebuilt.activeRuns;
      state.taskflowRecentRuns = rebuilt.recentRuns;
    } catch (error) {
      logger?.warn?.(`[mission-deck] failed to rebuild dashboard from taskflow: ${error?.message || error}`);
    }
    try {
      state.rebuiltDirectSessions = await loadDashboardDirectSessions(options);
    } catch (error) {
      logger?.warn?.(`[mission-deck] failed to rebuild direct sessions: ${error?.message || error}`);
    }
  ...
  async function attachChildOutcome(outcome) {
    ...
    if (activeRun && isDuplicateTerminalChildOutcome(activeRun, outcome)) return;
    if (activeRun) {
    ...
    state.recentRuns = state.recentRuns.map((run) => {
      ...
      if (isDuplicateTerminalChildOutcome(run, outcome)) return run;
      const nextRun = { ...run };
